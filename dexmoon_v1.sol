...
interface ICommunityRewards {
    function transferCallback() external;
}
...
struct Vaults {
    ...
    address communityBoostVault;  //GORDON Community Rewards
}
...
// 0xE53770B3735B1210372A127597506A33f5aB7755 charity       0,50%
// 0xeB2c48bcb539d96A7a00A85fc6dd423675D3E2BB dev           0,30%
// 0x535a8c0404c80f812F3f566066eb6847B97f5838 twok rewards  0,10%
// 0x3ca81C1951794A935F2A7e19Ed0CaDE486cd612A devtresury twok token (to be tax free)
// 0x10ED43C718714eb63d5aA57B78B54704E256024E pancakeswapRouter

contract Dexmmon is IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;

    string public constant name = "Dexmoon";                                
    string public constant symbol = "DXM";                                  
    uint8 public constant decimals = 9;
    uint256 public constant TOTAL_SUPPLY = 1*10e23;
    uint256 public constant _MAX_UINT = ~uint256(0);                        
    uint16 private constant _HUNDRED_PERCENT = 1000;                        
    uint256 public _TOTAL_REFLECTION = (_MAX_UINT - _MAX_UINT.mod(TOTAL_SUPPLY)); 

    TokenStats public _stats = TokenStats(_TOTAL_REFLECTION, TOTAL_SUPPLY, 0, 0, 0, 0); 
    TaxRates public _taxRates = TaxRates(60, 5, 3, 20, 1, 1, 90);           
    Vaults public _vaults;                                                  
    ...
    ICommunityRewards public communityRewards;

    bool public fairLauch = false;
    uint256 public fairDateStart = 0;
    uint256 public fairDateEnd = 0;
    uint256 public fairAmount = 0;
    ...
    function transferFrom(
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "Dexmoon: transfer amount exceeds allowance"));
        return true;
    }

    function approve(address spender, uint256 amount) external override returns (bool) {
        require((amount == 0) || (_allowances[_msgSender()][spender] == 0), "Dexmoon: approve from non-zero to non-zero allowance");
    ...
    function decreaseAllowance(address spender, uint256 value) external returns (bool) {
        ...
        require(oldValue >= value, "Dexmoon: cannot decrease allowance below zero");
    ...
    function setCommunityRewards(address rewards) external onlyOwner {
        communityRewards = ICommunityRewards(rewards);

        emit CommunityBoosterUpdated(rewards);
    }

    function updateLiquidityThreshold(uint256 threshold) external onlyOwner {
        require(threshold > 0, "Dexmoon: Cannot set threshold to zero");
    ...
    function reflectionFromToken(uint256 amountTokens, bool deductFees) public view returns (uint256) {
        require(amountTokens <= _stats.totalTokens, "Dexmoon: amount must be less than total supply");
    ...
    function tokenFromReflection(uint256 reflectionAmount) internal view returns (uint256) {
        require(reflectionAmount <= _stats.totalReflection, "Dexmoon: amount has to be less or equal to total reflection");
    ...
    function calculateActualSupply() public view returns (uint256, uint256) {
    ...
    function extendedTransferStandard(
        ...
        _balances[sender].reflection = _balances[sender].reflection.sub(
            ...
            "Dexmoon: transfer amount exceeds balance"
        );
    ...
    function extendedTransferExcluded(
        ...
        if (isToExcluded && isFromExcluded) {
            ...
            _balances[sender].tokens = _balances[sender].tokens.sub(amount, "Dexmoon: transfer amount exceeds balance");
            _balances[recipient].reflection = _balances[recipient].reflection.add(params.reflectionTransferAmount);
    ...
    function _transfer(
        ...
        require(to != address(0), "Dexmoon: transfer to the zero address");
        require(from != address(0), "Dexmoon: transfer from the zero address");
        require(amount > 0, "Dexmoon: Transfer amount must be greater than zero");

        if (fairLauch && ((block.timestamp >= fairDateStart) && (block.timestamp <= fairDateEnd))) {
            require(amount<= fairAmount, "Dexmoon: amount above the threshold");
        }
        
        // Reflection internal transfer to all participants and vaults
        ...
        if (_isUpdatingHolderCount) communityRewards.transferCallback();
    ...
    function _approve(
        ...
        require(spender != address(0), "Dexmoon: approve to the zero address");
        require(owner != address(0), "Dexmoon: approve from the zero address");
    ...
    function setFairLaunch(bool _fairLaunch, uint256 _fairDateStart, uint256 _fairDateEnd, uint256 _fairAmount) external onlyOwner {
        fairLauch = _fairLaunch;
        fairDateStart = _fairDateStart;
        fairDateEnd = _fairDateEnd;
        fairAmount = _fairAmount;
    }
   
}

contract CommunityRewards is Ownable {
    address public dexmoon;                                                 //GORDON dexmoon ERC20 Address
    address public twokToken;                                               //GORDON twok    ERC20 Address
    address public twokReward;                                              //GORDON twok Rewards Address
    IPancakeRouter02 public router;                                         //GORDON da aggiungere immutable come parola chiave
    
    bool public transferCallbackActivated;
    
    uint256 public amountReceived;
    
    modifier onlyDexmoon() {
        require(msg.sender == dexmoon, "CommunityBooster: FORDBIDDEN");
        _;
    }

    constructor(address pancakeRouter, address _dexmoon, address _twokReward, address _twokToken, address _owner) {
        IPancakeRouter02 _router = IPancakeRouter02(pancakeRouter);   //GORDON da abilitare, creazione del router
        router = _router;
        // BEP20
        dexmoon = _dexmoon;
        twokToken = _twokToken;
        
        // Multisig twok Reward
        twokReward = _twokReward;
        
        transferCallbackActivated = true;
    ...
    /*
    function transferCallback() external onlyDexmoon {
        if (transferCallbackActivated) {
            address[] memory path = new address[](3);
            path[0] = dexmoon;
            path[1] = router.WETH();
            path[2] = twokToken;
            
            IERC20 tokenDexmoon = IERC20(dexmoon);
            uint256 tokenAmount = tokenDexmoon.balanceOf(address(this));
            tokenDexmoon.approve(address(router), tokenAmount);
        
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAmount, 0, path, twokReward, block.timestamp);
        }
    }
    ...
    */
    ...
    function transferCallback() external onlyDexmoon {
        if (transferCallbackActivated) {
            ...
            tokenDexmoon.transfer(twokToken, 1);
        }
    }
  
    
    function setTokenAddress(address _dexmoon, address _twok, address _twokReward) external onlyOwner {
        dexmoon = _dexmoon;
        twokToken = _twok;
        twokReward = _twokReward;
    }
